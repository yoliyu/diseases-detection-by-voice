...
def _trainModel(X,y,foldsNumber:int, model,standarized:bool,dimensionReduction:str,dimensionReductionComponents):
    ...
    actual_classes, predicted_classes, _, avg,pre,f1 = __crossKStratifiedValidation(model, foldsNumber, X, y,standarized,dimensionReduction,dimensionReductionComponents)
    __plotConfusionMatrix(actual_classes, predicted_classes, [0, 1])
    return avg,pre,f1
...
def __crossKStratifiedValidation(model, k, X, y,standarized,dimensionReduction, dimensionReductionComponents) -> Tuple[np.array, np.array, np.array]:
    ...
    return actual_classes, predicted_classes, predicted_proba, avg_acc_score, avg_pre_score, avg_f1_score
