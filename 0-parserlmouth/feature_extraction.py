...
sex = meta['Sex'].tolist()
for wave_file in glob.glob("../bd/SVD/BD/A_NEUTRAL_NOTES/*.wav"):
    
    x = wave_file.replace("../bd/SVD/BD/A_NEUTRAL_NOTES/", "")
    ...
    df = pd.DataFrame(np.column_stack([file_list, sex_list, healthy_list, mean_F0_list, sd_F0_list, meanHNR_list, stdevHNR_list, 
                                   ...
                                   f4_median_list]),
                                   columns=['voiceID', 'sex', 'healthy', 'meanF0Hz', 'stdevF0Hz', 'meanHNR', 'stdevHNR', 
